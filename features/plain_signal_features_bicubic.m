...
function [train_data, test_data, train_lbl, test_lbl] = plain_signal_features_bicubic(p, train_trials, test_trials, train_labels, test_labels)
	...
	function [to,labels] = compute(from, labels)
		...
		for i=1:trials
			raw_eeg = from(:, :, i);
            to(:, :, i) = imresize(raw_eeg, p.cnn_img_size, 'bicubic');
		end
