...
function [train_data, test_data, train_lbl, test_lbl] = plain_fft_features(p, train_trials, test_trials, train_labels, test_labels)
	...
	function [to,labels] = compute(from, labels)
		...
		for i=1:trials
            ...
            to(:, :, i) = imresize(temp_im, p.cnn_img_size); % no nearest - need more smoothing
        end
