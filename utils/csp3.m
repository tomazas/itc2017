function [V,d] = csp3(ECM,k_pat,Mode)
	...
	if strcmpi(type,'CSP0')
		...
		for k = 1:sz(1), 
			...
			[R,d1]  = eig((C+C')/2);
			[d1,ix] = sort(diag(d1)); 
			V(:,2*pat*k+[1-2*pat:0]) = P'*R(:,ix([1:pat,end-pat+1:end]));
			d(1,2*pat*k+[1-2*pat:0]) = d1(ix([1:pat,end-pat+1:end]))';
		end; 
	elseif strcmpi(type,'CSP3')
